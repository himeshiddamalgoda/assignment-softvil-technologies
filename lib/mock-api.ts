...
const createResponse = async <T>(data: T, status = 200, delayMs = 800)
...
export const eventApi = {
  ...
  attendEvent: async (eventId: string, userId: string) => {
    try {
      ...
      const attendeeIndex = mockEvents[eventIndex].attendees.findIndex((a:Attendee) => a.userId === userId)
      ...
      if (attendeeIndex !== -1) {
        ...
        updatedEvent = {
          ...
          attendees: [
            ...
            { userId, userName: user.name, avatarUrl: user.avatarUrl, status: "confirmed" },
          ],
          updatedAt: new Date().toISOString(),
        }
      }
